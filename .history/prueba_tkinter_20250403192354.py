...
# Se agregó un parámetro opcional "callback" que se ejecuta al cerrar el mensaje.
def mostrar_mensaje_custom(titulo, mensaje, callback=None):
    ...
    tk.Button(mensaje_win, text="Aceptar", font=("Century Gothic", 12, "bold"),
              bg="#157347", fg="white",
              command=lambda: [mensaje_win.destroy(), callback() if callback else None]).pack(pady=10)
...
def realizar_compra_gui():
    ...
    def finalizar_compra():
        ...
        try:
            ...
            # Se muestra un mensaje de éxito y se cierra la ventana actual, retornando al menú
