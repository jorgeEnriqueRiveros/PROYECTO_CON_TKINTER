...
def cargar_imagen_fondo(ancho, alto):
    # Si se encuentra la imagen, se redimensiona; de lo contrario, se genera una imagen gris
...
# Función personalizada para mostrar mensajes con un botón "Aceptar".
# Se agregó el parámetro opcional "callback" para ejecutar una acción al cerrar el mensaje.
def mostrar_mensaje_custom(titulo, mensaje, callback=None):
    ...
    tk.Button(mensaje_win, text="Aceptar", font=("Century Gothic", 12, "bold"),
              bg="#157347", fg="white", command=lambda: [mensaje_win.destroy(), callback() if callback else None]).pack(pady=10)

# Función auxiliar para crear ventanas de consulta con un formato consistente
...
def realizar_compra_gui():
    ...
    # Botón para finalizar la compra y guardarla en la base de datos
