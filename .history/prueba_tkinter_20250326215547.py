...
import os
from funciones import (registrar_persona, registrar_producto, comprar, 
                       ver_info_cliente, ver_compras_cliente, 
                       sumar_compras_cliente, ordenar_productos_mas_vendidos)
...
def abrir_nueva_ventana():
    ...
    botones = [
        ("Registrar Persona", registrar_persona),
        ("Registrar Producto", registrar_producto),
        ("Comprar", comprar),
        ("Ver Información Cliente", ver_info_cliente),
        ("Ver Compras Cliente", ver_compras_cliente),
        ("Sumar Valor Compras", sumar_compras_cliente),
        ("Ordenar Más Vendidos", ordenar_productos_mas_vendidos),
        ("Salir", nueva_ventana.destroy)
    ]

    for texto, comando in botones:
        boton = tk.Button(frame_consultas, text=texto, font=("Century Gothic", 12), bg="#007BFF", fg="white", command=comando)
