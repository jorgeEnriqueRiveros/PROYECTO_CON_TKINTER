...
# Establecer transparencia global de la ventana (opcional)
windows.attributes('-alpha', 0.97)  # Casi completamente opaca (1.0 = sin transparencia)
...
# Simulación de transparencia en el Frame usando una imagen con opacidad reducida
ruta_frame = r"D:\PROYECTO_CON_TKINTER\Images\frame_transparente.png"
if os.path.exists(ruta_frame):
    img_frame = Image.open(ruta_frame).convert("RGBA")
    
    # Reducir opacidad al 30%
    alpha = 0.3
    img_frame = ImageEnhance.Brightness(img_frame).enhance(alpha)
    
    img_frame = img_frame.resize((350, 300))
    img_frame = ImageTk.PhotoImage(img_frame)

    # Agregar imagen semi-transparente como fondo del Frame
...
frame = tk.Frame(windows, bg="#053A20", bd=5)  # Color oscuro para simular transparencia
